...
if ! command -v kano-backlog >/dev/null 2>&1; then
  echo "WARNING: kano-backlog CLI not available - CLI docs will be limited"
fi
...
if command -v kano-backlog >/dev/null 2>&1; then
  ...
  echo "" >> "$BUILD_DIR/content_quartz/cli/commands.md"
  echo "Complete command-line interface documentation for the kano-backlog tool." >> "$BUILD_DIR/content_quartz/cli/commands.md"
  echo "" >> "$BUILD_DIR/content_quartz/cli/commands.md"
  echo "## kano-backlog" >> "$BUILD_DIR/content_quartz/cli/commands.md"
  echo '```' >> "$BUILD_DIR/content_quartz/cli/commands.md"
  kano-backlog --help >> "$BUILD_DIR/content_quartz/cli/commands.md" 2>/dev/null || echo "Command not available" >> "$BUILD_DIR/content_quartz/cli/commands.md"
  echo '```' >> "$BUILD_DIR/content_quartz/cli/commands.md"
else
  echo "---" > "$BUILD_DIR/content_quartz/cli/commands.md"
  echo "title: CLI Commands" >> "$BUILD_DIR/content_quartz/cli/commands.md"
  ...
  echo "# CLI Commands" >> "$BUILD_DIR/content_quartz/cli/commands.md"
  echo "" >> "$BUILD_DIR/content_quartz/cli/commands.md"
  echo "kano-backlog command not available" >> "$BUILD_DIR/content_quartz/cli/commands.md"
fi
